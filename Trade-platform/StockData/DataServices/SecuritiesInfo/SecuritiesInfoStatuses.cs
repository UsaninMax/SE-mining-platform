namespace TradePlatform.StockData.DataServices.SecuritiesInfo
{
    //TODO: from property file
    public static class SecuritiesInfoStatuses
    {
        public static string FailToUpdateSecuritiesInfo => "Fail to update securities info";

        public static string SecuritiesInfoUpdated => "Securities info - updated";

        public static string SecuritiesInfoUpdateInProgress => "Securities info update - in progress";
    }
}
